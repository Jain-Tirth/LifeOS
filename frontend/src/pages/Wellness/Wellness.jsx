    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {Object.entries(typeCount).map(([type, count]) => (
          <div
            key={type}
            className={`p-4 rounded-2xl border border-white/10 ${activityColors[type]?.split(" ")[1] || "bg-white/5"}`}
          >
            <div className="flex items-center gap-2 mb-2">
              <div
                className={`p-1.5 rounded-lg ${activityColors[type] || "text-white/40 bg-white/5"}`}
              >
                {activityIcons[type] || <Activity size={16} />}
              </div>
            </div>
            <div
              className={`text-2xl font-bold font-display ${activityColors[type]?.split(" ")[0] || "text-white"}`}
            >
              {count}
            </div>
        ...
        <div className="p-6 rounded-3xl glass-card bg-teal-500/5 border-teal-500/10 relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="text-4xl font-bold text-white font-display mb-1">
              {typeCount.hydration || 0}
              <span className="text-lg text-white/50 ml-1">logs</span>
            </div>
            <p className="text-white/40 text-sm">
              Keep logging to track your intake
            </p>
          </div>
                ...
                <BarChart data={last7Days}>
                  <Tooltip
                    ...
                    formatter={(value, name) => [
                      value,
                      name === "minutes" ? "Minutes" : "Activities",
                    ]}
                  />
        ...
        <div className="flex border-b border-white/10 overflow-x-auto scrollbar-hide">
          {types.map((type) => (
            <button
              ...
              className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors capitalize relative ${
                activeType === type
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-white/20 text-sm mt-1">
                Use the Orchestrator chat or click "Log Activity" to add one.
              </p>
            </div>
          ) : (
            <AnimatePresence>
              {filteredActivities.map((activity) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-full ${activityColors[activity.activity_type] || "text-white/40 bg-white/5"}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/40 max-w-md truncate">
              ...
              <form onSubmit={handleAddActivity} className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setAddForm((p) => ({ ...p, activity_type: type }))
                        }
                        className={`p-3 rounded-xl text-sm font-medium border transition-all capitalize flex flex-col items-center gap-1 ${
                          addForm.activity_type === type
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddForm((p) => ({ ...p, recorded_at: e.target.value }))
                    }
                    className="w-full glass-input rounded-xl px-4 py-3"
                  />
                </div>
